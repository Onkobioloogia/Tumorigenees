...
eurorate <- read.csv("eurorate.csv") %>% 
  melt(.,id="Vanusegrupp") %>% 
  filter(value > 2 & !variable == "Hodgkin.s.lümfoom")
cancers <- eurorate  %>%
  group_by(variable) %>%
  nest %>% 
  mutate(model = map(data, ~lm(log(value) ~ log(Vanusegrupp), data = .)),
...
ggplot(eurorate, aes(x = Vanusegrupp, y = value)) + 
  ...
  geom_text(data = cancers, aes(x = 25, y = y, label = sprintf('%2.1f', slope)), size = 4, hjust = 0) +
  xlab("Vanus, aastat") + 
...
- Keskmine tõus kõrvalolevate andmete põhjal on **`r sprintf('%2.1f', mean(cancers$slope))`** (vahemik: *`r sprintf('%2.1f', range(cancers$slope))`*).
